...
if($_GET["cmd"] == "edit" && is_numeric($_GET["id"]) && !empty($_GET["id"])){
  ...
  // Server rebooten
  reboot_server(mysql_real_escape_string($_GET["id"]));
}elseif($_GET["cmd"] == "shutdown" && is_numeric($_GET["id"]) && !empty($_GET["id"])){
  // Server herunterfahren
  shutdown_server(mysql_real_escape_string($_GET["id"]));
}elseif($_GET["cmd"] == "shutdown_all"){
  // Alle Server herunterfahren
  $query = mysql_query("SELECT id FROM server");
  while($row = mysql_fetch_assoc($query)) shutdown_server($row["id"]);
}

// Formular wurde abgeschickt
if($_POST["add"] || $_POST["edit"]){
  if(empty($_POST["ip"]) || empty($_POST["name"])){ // IP und Name duerfen nicht leer sein
    echo "<div class='meldung_error'>IP und Name m&uuml;ssen angegeben werden!</div><br>";
    $display = "block";
    $value = $_POST;
    if($_POST["edit"]){
      $submit_name = "edit";
    ...
    if($_POST["add"]){
      // Server hinzufuegen
      mysql_query("INSERT INTO server SET name = '".$name."', ip = '".$ip."', score = '".$score."', games = '".$games."'");
      ...
      // Server aendern
      mysql_query("UPDATE server SET name = '".$name."', ip = '".$ip."', score = '".$score."', games = '".$games."' WHERE id = '".$id."' LIMIT 1");
    }
  }
}
...
while($row = mysql_fetch_assoc($query)){
  if(host_online($row["ip"])) $server_color = "#00FF00"; // Host online? Farbe anpassen
  else $server_color = "#FF0000";
  echo "<tr>
    <td style='background-color: $server_color;'>&nbsp;</td>
...
// SSH-Key anzeigen
echo "<h3>SSH-Key</h3>";
echo "Folgender ssh-Key muss auf allen Server in der Datei /root/.ssh/authorized_keys eingetragen werden! (ist nur eine Zeile!)<br>";
